...
/**
 * Controller function to increment the returns count for a product.
 *
...
export const incrementReturns = async (req: Request, res: Response) => {
  try {
    ...
    // Increment the returns count by 1 and set a unique analyticsId if inserting a new document
    const analytics = await ProductAnalytics.findOneAndUpdate(
      ...
      {
        $inc: { returns: 1 }, // Increment the returns count
